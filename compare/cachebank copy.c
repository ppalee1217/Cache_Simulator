  ...
  if(cache_bank[choose].stall){
    ...
    if(req_queue_empty(request_queue)){
     // printf("Request queue is empty, No need to send.\n");
      ...
      int maf_result = cacheset_access(wb, cache_bank[choose].cache_set, cache, choose, request_queue->request_addr[0], request_queue->request_type[0], 0, &hits, &misses, &writebacks, ADDRESSING_MODE);
